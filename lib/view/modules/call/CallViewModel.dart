      ...
      if(_currentState.charactersUid != value)
{
  ...
  Future<void> getToken([bool reload = false]) async {
    ...
    _configService.getToken().then((value) {
      streamController.add(_currentState.copy(TokenLoaded(
          value)));
    }).onError((error, stackTrace) {
      streamController.add(
          _currentState.copy(CallFailed(error.toString())));
  ...
  Future<void> join(int uid) async {
    print("JOINJOIN ${uid}");
    String characterName = this._currentState.uiState.pjDisplay ? (await this._configService.getCharacterName() ?? '') : '';
    print("JOINJOIN ${characterName}");

    if(characterName != '')
    _configService.join(characterName, uid);
